 ...
 */
package visao.view.com.inet.jortho;
...
/**
 * This class is the major class of the spell checker JOrtho (Java Orthography
 * Checker). In the most cases this is the only class that you need to add spell
 * checking to your application. First you need to do a one-time registration of
 * your dictionaries. In standalone applications this can look like:
 * <code><pre>
 * SpellChecker.registerDictionaries( new URL("file", null, ""), "en,de", "de" );
 * </pre></code> and in an applet this will look like:
 * <code><pre>
 * SpellChecker.registerDictionaries( getCodeBase(), "en,de", "en" );
 * </pre></code> After this you can register your text component that should
 * have the spell checker features (Highlighter, context menu, spell checking
 * dialog). This looks like:
 * <code><pre>
...
public class SpellChecker {
    ...
    /**
     * Sets the UserDictionaryProvider. This is needed if the user should be
     * able to add their own words. This method must be called before
     * {@link #registerDictionaries(URL, String, String)}.
     *
    ...
    /**
     * Gets the currently set UserDictionaryProvider. If none has been set then
     * null is returned.
     *
    ...
    /**
     * Set a CustomDictionaryProvider. This can be used to add an expert
    ...
    /**
     * Gets the currently set CustomDictionaryProvider. If none has been set
     ...
     * the form "dictionary_xx.xxxxx" and must be relative to the baseURL. The
     * available languages and extension of the dictionaries is load from a
     * configuration file. The configuration file must also relative to the
     * baseURL and must be named dictionaries.cnf, dictionaries.properties or
     * dictionaries.txt. If the dictionary of the active Locale does not exist,
     * the first dictionary is loaded. There is only one dictionary loaded in
     * memory at a given time. The configuration file has a Java Properties
     * format. Currently there are the follow options: <ul> <li>languages</li>
     * <li>extension</li> </ul>
     *
     * @param baseURL the base URL where the dictionaries and configuration file
     * can be found. If null then URL("file", null, "") is used.
     * @param activeLocale the locale that should be loaded and made active. If
     * null or empty then the default locale is used.
     */
    ...
    /**
     * Registers the available dictionaries. The dictionaries' URLs must have
     * the form "dictionary_xx.ortho" and must be relative to the baseURL. If
     * the dictionary of the active Locale does not exist, the first dictionary
     * is loaded. There is only one dictionary loaded in memory at a given time.
     *
     * @param baseURL the base URL where the dictionaries can be found. If null
     * then URL("file", null, "") is used.
     ...
     * Registers the available dictionaries. The dictionaries' URLs must have
     * the form "dictionary_xx.xxxxx" and must be relative to the baseURL. The
     * extension can be set via parameter. If the dictionary of the active
     * Locale does not exist, the first dictionary is loaded. There is only one
     * dictionary loaded in memory at a given time.
     *
     * @param baseURL the base URL where the dictionaries can be found. If null
     * then URL("file", null, "") is used.
     * @param availableLocales a comma separated list of locales
     * @param activeLocale the locale that should be loaded and made active. If
     * null or empty then the default locale is used.
     * @param extension the file extension of the dictionaries. Some web server
     * like the IIS6 does not support the default ".ortho".
     * @see #setUserDictionaryProvider(UserDictionaryProvider)
     */
     ...
     * <code>JTextComponent</code>. The call is equal to register( text, true,
     * true ).
     *
     ...
     * <code>JTextComponent</code>. You do not need to unregister if the
     * JTextComponent is not needed anymore.
     *
     * @param text the JTextComponent
     * @param hasPopup if true, the JTextComponent is to have a popup menu with
     * the menu item "Orthography" and "Languages".
     * @param hasShortKey if true, pressing the F7 key will display the spell
     * check dialog.
     * @param hasAutoSpell if true, the JTextComponent has a auto spell
     * checking.
     * @throws NullPointerException if text is null
     */
    ...
    /**
     * Removes all spell checker features from the JTextComponent. This does not
     * need to be called if the text component is no longer needed.
     *
     * @param text the JTextComponent
     */
    ...
    /**
     * Enable or disable the F7 key. Pressing the F7 key will display the spell
     * check dialog. This also register an Action with the name
     * "spell-checking".
     *
    ...
    /**
     * Show the Spell Checker dialog for the given JTextComponent. It will be do
     * nothing if the JTextComponent is not editable or there are no dictionary
     * loaded. The action for this method can you receive via:
     * <code><pre>
     * Action action = text.getActionMap().get("spell-checking");
     * </pre></code> The action is only available if you have enable the short
     * key (F7).
     *
    ...
    /**
     * Enable or disable the auto spell checking feature (red zigzag line) for a
     * text component. If you change the document then you need to reenable it.
     *
     * @param text the JTextComponent that should change
     * @param enable true, enable the feature.
     */
    ...
    /**
     * Adds a LanguageChangeListener. You do not need to remove it if the
     * LanguageChangeListener is not needed anymore. You need a hard reference
     * to the listener because the SpellChecker hold only a WeakReference.
     *
    ...
    /**
     * Creates a menu item "Orthography" (or the equivalent depending on the
     * user language) with a sub-menu that includes suggestions for a correct
     * spelling. You can use this to add this menu item to your own popup.
     *
     * @return the new menu.
     */
    ...
    /**
     * Create a dynamic JPopupMenu with a list of suggestion. You can use the
    ...
    /**
     * Creates a menu item "Languages" (or the equivalent depending on the user
     * language) with a sub-menu that lists all available dictionary languages.
     * You can use this to add this menu item to your own popup or to your menu
     * bar.
     * <code><pre>
    ...
    public static JMenu createLanguagesMenu(SpellCheckerOptions options) {
        JMenu menu = new JMenu(Utils.getResource("Linguagens"));
    ...
    /**
     * Base class for languages change actions. This class has a static state to
     * solv http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4133141
     */
    private static abstract class AbtsractLanguageAction extends AbstractAction {
         ...
         *
         * @param b true selects the menu item, false deselects the menu item.
         */
        public void setSelected(boolean b) {
            if (b) {
    ...
    /**
     * Get the current
     * <code>Dictionary</code>. The current dictionary will be set if the user
     * one select or on calling
     * <code>registerDictionaries</code>.
     *
    ...
    /**
     * Set the title of your application. This valuse is used as title for info
     * boxes (JOptionPane). If not set then the translated "Spelling" is used.
     */
    ...
    /**
     * Get the default SpellCheckerOptions. This object is a singleton. That
